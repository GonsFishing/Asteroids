...
def main():
    ...
    while True:
        ...
        for asteroid in asteroids:
            if asteroid.collides_with_circle(player) and player.invincible == False:
                print("Game over!")
                set_score(score)
                sys.exit(0)
            for shot in shots:
        ...
        for object in drawables:
            object.draw(screen)
        draw_text(screen, f'Score: {score}', font, "white", SCREEN_WIDTH - (SCREEN_WIDTH/8), SCREEN_HEIGHT - (SCREEN_HEIGHT/8))
